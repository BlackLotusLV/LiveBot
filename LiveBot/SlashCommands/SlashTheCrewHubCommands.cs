...
public sealed partial class SlashTheCrewHubCommands : ApplicationCommandModule
{
    ...
    public async Task TopSummit(InteractionContext ctx, [Option("platform", "Which platform leaderboard you want to see")] Platforms platform = Platforms.Pc)
    {
        ...
        await Parallel.ForEachAsync(jSummit[0].Events, new ParallelOptions(), async (summitEvent, _) =>
        {
            ...
            int i = jSummit[0].Events.Select((element, index) => new { element, index })
                .FirstOrDefault(x => x.element.Equals(summitEvent))?.index ?? -1;
            var activity = JsonConvert.DeserializeObject<SummitLeaderboard>(
                await wc.GetStringAsync($"https://api.thecrew-hub.com/v1/summit/{jSummit[0].Id}/leaderboard/{search}/{summitEvent.Id}?page_size=1", CancellationToken.None));
            Rank rank = null;
            if (activity.Entries.Length != 0)
            {
                rank = JsonConvert.DeserializeObject<Rank>(await wc.GetStringAsync($"https://api.thecrew-hub.com/v1/summit/{jSummit[0].Id}/score/{search}/profile/{activity.Entries[0].ProfileId}",
        ...
        long weekNumber = 0)
    {
        ...
        Parallel.For(0, rewards.Length, (i, _) =>
        {
            ...
            var rewardImage = Image.Load<Rgba32>(TheCrewHubService.RewardsImagesBytes[(int)week, i]);
    ...
    public async Task LinkHub(InteractionContext ctx, [Option("link", "Your Ubisoft avatar link.")] string link, [Option("platform", "The platform you want to link")] Platforms platform)
    {
        ...
        if (!Guid.TryParse(link, out _))
        {
            await ctx.EditResponseAsync(
                new DiscordWebhookBuilder().WithContent("The link you provided does not contain your Ubisoft Guid. Please check the tutorial again of how to get the right link"));
            return;
        }
        ...
        if (info != null)
        {
            ...
            {
                await ctx.EditResponseAsync(new DiscordWebhookBuilder().WithContent("The Ubisoft account you are trying to link has already been linked with a different Discord account."));
                return;
            }

            await ctx.EditResponseAsync(new DiscordWebhookBuilder().WithContent("These accounts have already been linked"));
            return;
        }

        UbiInfo newEntry = new(ctx.User.Id)
        {
            ProfileId = link,
            Platform = search
        };
    ...
    public enum Week
    {
        [ChoiceName("This Week")] ThisWeek = 0,

        [ChoiceName("Next Week")] NextWeek = 1,

        [ChoiceName("3rd Week")] ThirdWeek = 2,

        [ChoiceName("4th Week")] ForthWeek = 3
    }
