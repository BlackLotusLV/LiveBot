...
internal sealed class SlashCommands : ApplicationCommandModule
{
    ...
    public async Task Cookie(InteractionContext ctx, [Option("User", "Who to give the cookie to")] DiscordUser member)
    {
        ...
        {
            await ctx.EditResponseAsync(new DiscordWebhookBuilder().WithContent(
                $"Your cookie box is empty. You can give a cookie in {24 - DateTime.UtcNow.Hour} Hours, {59 - DateTime.UtcNow.Minute - 1} Minutes, {59 - DateTime.UtcNow.Second} Seconds."));
            return;
        }
    ...
    public async Task SubmitPhoto(InteractionContext ctx,
        [Autocomplete(typeof(PhotoContestOption)), Option("Competition", "To which competition to submit to.")]
        long competitionId,
